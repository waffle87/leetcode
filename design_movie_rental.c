...
static void av_split(AvNode *root, int price, int shop, AvNode **left,
                     AvNode **right) {
  if (!root) {
    *left = *right = NULL;
    return;
...
static void r_split(RNode *root, int price, int shop, int movie, RNode **left,
                    RNode **right) {
  if (!root) {
    *left = *right = NULL;
    return;
...
static RNode *r_insert(RNode *root, RNode *node) {
  ...
  if (node->pr < root->pr) {
    ...
    if (cmp_r(node->price, node->shop, node->movie, root->price, root->shop,
              root->movie) < 0)
...
static AvNode *new_avnode(int price, int shop) {
  ...
  p->price = price;
  p->shop = shop;
  p->pr = ((unsigned)rand() << 16) ^ (unsigned)rand();
  p->l = p->r = NULL;
  return p;
}
...
MovieRentingSystem *movieRentingSystemCreate(int n, int **entries,
                                             int entriesSize,
                                             int *entriesColSize) {
  srand(1234567);
  MovieRentingSystem *obj =
      (MovieRentingSystem *)malloc(sizeof(MovieRentingSystem));
  obj->n = n;
  obj->m = entriesSize;
  obj->priceMap = hashmapCreate((size_t)entriesSize + 4);
  obj->available = (AvNode **)calloc(MAX_SIZE, sizeof(AvNode *));
  obj->rentedRoot = NULL;
...
int *movieRentingSystemSearch(MovieRentingSystem *obj, int movie,
                              int *retSize) {
  int *res = (int *)malloc(sizeof(int) * 5);
  *retSize = 0;
  if (!obj)
    return res;
  if (movie < 0 || movie >= MAX_SIZE)
    return res;
...
int **movieRentingSystemReport(MovieRentingSystem *obj, int *retSize,
                               int **retColSize) {
  int **res = (int **)malloc(sizeof(int *) * 5);
...
void movieRentingSystemFree(MovieRentingSystem *obj) {
  ...
  if (obj->available) {
    for (int i = 0; i < MAX_SIZE; ++i)
      if (obj->available[i])
        av_free(obj->available[i]);
    free(obj->available);
  }
  if (obj->rentedRoot)
    r_free(obj->rentedRoot);
  if (obj->priceMap) {
...
int main() {
  int entries_input[6][3] = {{0, 1, 5}, {0, 2, 6}, {0, 3, 7},
                             {1, 1, 4}, {1, 2, 7}, {2, 1, 5}};
  int retSize, *retColSize;
  struct two_d_arr *entries = two_d_arr_init(
      ARRAY_SIZE(entries_input), ARRAY_SIZE(entries_input[0]), entries_input);
  MovieRentingSystem *obj = movieRentingSystemCreate(
      3, entries->arr, entries->row_size, entries->col_size);
  int *s1 = movieRentingSystemSearch(obj, 1, &retSize);
  for (int i = 0; i < retSize; i++)
    printf("%d ", s1[i]);
  printf("\n");
  free(s1);
  movieRentingSystemRent(obj, 0, 1);
  movieRentingSystemRent(obj, 1, 2);
  int **r1 = movieRentingSystemReport(obj, &retSize, &retColSize);
  for (int i = 0; i < retSize; i++) {
  ...
  for (int i = 0; i < retSize; i++)
    free(r1[i]);
  free(r1);
  movieRentingSystemDrop(obj, 1, 2);
  int *s2 = movieRentingSystemSearch(obj, 2, &retSize);
  for (int i = 0; i < retSize; i++)
    printf("%d ", s2[i]);
  printf("\n");
  free(s2);
  two_d_arr_free(entries);
